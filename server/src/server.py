# -*- coding: utf-8 -*-
"""
Agora Agent & Token Service — Vision Recipe

HTTP APIs:
- GET  /get_config     -> Generate connection config
- POST /startAgent     -> Start vision agent
...
# FastAPI application
app = FastAPI(
    title="Agora Vision Recipe Service",
    version="1.0.0",
    description="Agora Conversational AI with vision (camera) support",
)
...
def _generate_channel_name() -> str:
    return f"vision-{int(time.time())}-{random.randint(1000, 9999)}"
...
async def start_agent(request: StartAgentRequest):
    """Start vision agent in a channel"""
    if agent is None:
        raise HTTPException(
            status_code=500,
            detail="Service not properly configured. Please check environment variables.",
